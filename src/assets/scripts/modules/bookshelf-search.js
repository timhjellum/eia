...
class BookshelfSearch {
  constructor() {
    ...
    $("#header-bookshelf-search-form", this.container).on("submit", function(
      e
    ) {
      self.submitSearch();
      e.preventDefault();
  ...
  handleSearchInput(e) {
    ...
    for (i = 0; i < this.tags.length; i++) {
      var initCap = this.tags[i].label.substring(0, 1);
      var remDsc = this.tags[i].label.substring(1, this.tags[i].label.length);
      var fullDsc = this.tags[i].label.toLowerCase();
